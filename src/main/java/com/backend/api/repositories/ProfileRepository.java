...
public interface ProfileRepository extends JpaRepository<UserProfile, Integer>, JpaSpecificationExecutor<UserProfile> {
  ...
  Page<Route> getRoutes(@Param("profileId") Integer profileId, Pageable pageable);

  @Transactional()
  @Query("SELECT x from UserProfile p JOIN p.routes x WHERE p.id = :profileId")
  List<Route> getRoutes(@Param("profileId") Integer profileId);

  @Transactional()
  @Query("SELECT x from UserProfile p JOIN p.users x WHERE p.id = :profileId")
  Page<User> getUsers(@Param("profileId") Integer id, Pageable pageRequest);

  @Transactional()
  @Query("SELECT x from UserProfile p JOIN p.users x WHERE p.id = :profileId")
  List<User> getUsers(@Param("profileId") Integer id);
